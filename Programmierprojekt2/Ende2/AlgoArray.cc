...
double mean(DynamicArray &dynamic_array)
{
	...
	{
		mean_value += dynamic_array.get_value(i);
	}
...
double sum(DynamicArray &dynamic_array)
{
	...
	for (int i = 0; i < length; i++)
	{
		sum_value += dynamic_array.get_value(i);
	}
...
double median(DynamicArray &dynamic_array)
{
	...
	if (is_odd)
	{
		...
		median_value = dynamic_array.get_value(index);
	}
	else
	{
		...
		median_value = 
			(dynamic_array.get_value(index1) + dynamic_array.get_value(index2)) / 2.0;
	}
...
double variance(DynamicArray &dynamic_array)
{
	...
	{
		double a = dynamic_array.get_value(i);
